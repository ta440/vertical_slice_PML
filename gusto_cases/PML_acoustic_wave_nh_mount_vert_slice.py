...
from gusto import (
    ...
    PMLParameters, Timestepper, RK4
)

savename = 'PML_acoustic_wave_vert_slice_rk4'
...
mu_dt = 0.3             # strength of sponge layer, no units
...
PML_pars = PMLParameters(mesh, H=domain_height)
...
transported_fields = [
    ...
    TrapeziumRule(domain, "q_u", subcycling_options=subcycling_opts),
    SSPRK3(
        domain, "q_rho", rk_formulation=RungeKuttaFormulation.predictor,
        subcycling_options=subcycling_opts
    ),
    SSPRK3(
        domain, "q_theta", options=theta_opts,
        subcycling_options=subcycling_opts
    )
]
transport_methods = [
    ...
    DGUpwind(eqns, "q_u"),
    DGUpwind(eqns, "q_rho"),
    DGUpwind(eqns, "q_theta", ibp=theta_opts.ibp)
...
q_rho0 = stepper.fields("q_rho")
q_theta0 = stepper.fields("q_theta")
...
zero_expr = Constant(0.0)*z

theta0.assign(theta_b)
rho0.assign(rho_b)
u0.project(as_vector([initial_wind, 0.0]))

q_theta0.interpolate(zero_expr)
q_rho0.interpolate(zero_expr)
q_u0.project(as_vector([zero_expr, 0.0]))
