...
results_dir = 'cs2023_nonhydrostatic_gaussian_h1000'
...
contours = [
    np.linspace(-w_max, w_max, 23), np.linspace(-theta_max, theta_max, 15),
    np.linspace(-w_max, w_max, 23), np.linspace(-theta_max, theta_max, 31)
]

field_names = ['u_z', 'theta_perturbation', 'u_z', 'theta_perturbation']
colour_schemes = ['PiYG', 'RdBu_r', 'PiYG', 'RdBu_r']
field_labels = [
    r'$w$ (m s$^{-1}$)', r'$\Delta\theta$ (K)',
    r'$w$ (m s$^{-1}$)', r'$\Delta\theta$ (K)'
]
...
for i, (ax, field_name, colour_scheme, field_label, contours) in \
        enumerate(zip(
            axarray.flatten(), field_names, colour_schemes,
            field_labels, contours
        )):
    ...
    if i in [0, 1]:
        ...
        tomplot_field_title(
            ax, None, minmax=True, field_data=field_data, minmax_format='.5f'
        )
...
plot_name = f'{plot_stem}_t{time:.0f}.png'
