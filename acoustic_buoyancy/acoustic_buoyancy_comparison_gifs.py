...
vanilla_results_dir = 'acoustic_buoyancy_no_topo_Gaussian_p_pert'
PML_results_dir = 'PML_acoustic_buoyancy_no_topo_Gaussian_p_pert'
extra_name = ''
...
savename = f'compare_{field_name}'
...
for i in np.arange(len(t_range)):
    ...
    plt.suptitle(f't={time:.1f} s \n')
    tomplot_field_title(ax1, f'Vanilla \n', minmax=True, field_data=vanilla_field_data)
    tomplot_field_title(ax2, f'PML \n', minmax=True, field_data=PML_field_data)
